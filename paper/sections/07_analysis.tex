...
The placebo and leakage controls (\Cref{sec:results:specificity}) rule out the two
deflationary readings of the cross-document $\Delta$nll. The derangement placebo
removes almost all of the gain while holding the amount of granted context fixed, so
the benefit is specificity to the correct linked document, not generic extra context;
and the reduction persists in the lowest $n$-gram-overlap stratum, so it is not
verbatim copying of duplicated text. What remains is the intended inductive bias:
attention gated onto the right cross-document target at generation time.
...
\emph{Leakage.} HotpotQA is built on 2017 English Wikipedia and The Stack overlaps
public code; the paired same-token contrast cancels memorization of the completion,
and the $\alpha$-stratified analysis shows the gain survives where the completion
cannot be copied from the granted document (\Cref{sec:results:specificity}) --- but
corpus deduplication itself remains sampling-only. \emph{Fired-subset reporting.} $\Delta$ is over items where the
