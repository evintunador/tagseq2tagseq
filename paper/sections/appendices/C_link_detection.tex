...
template/multiline string bodies without a full grammar. The \emph{shipped} graph is
instead extracted offline with tree-sitter grammars, and an independent tree-sitter
oracle grades the runtime detector for agreement, giving a three-way check of the
token-space approximation against two grammar-based references
\citep{brunsfeld2018treesitter}.
...
\subsection{Training-time shortcut vs.\ generation-time detection}

At training time the packer can bypass detection entirely: the resolved graph edge
is baked directly into the packed example, so the online detector is a fallback
rather than the primary path. Text detection is therefore really a
\emph{generation-time and grader-time} concern: at inference there are no baked
edges, so the detectors run over the model's own output (see the link-following
inference procedure), and the grader runs them to score whether a generated link
resolves.
