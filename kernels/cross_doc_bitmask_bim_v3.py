...
def _attn_fwd_cdb_bim_v3(
    ...
    # Bitmask-only (_attn_fwd_inner_cross_v3) is unsafe when a KV block
    # straddles a doc boundary — part of it belongs to Q's own doc, so
    # same_doc can be True for some position pairs.  Use the full v1 inner
    # kernel (same_doc | in_grant, DIAGONAL=False) for correctness.
    # The fast "full" path above is still valid: both blocks are confirmed
    # single-doc and same-doc, so every position attends.
    for i in range(n_full, num_kv - 1):
        ...
        O, L, M = _attn_fwd_inner_cdb_bim_v1(
            ...
            BLOCK_SIZE_QO, BLOCK_SIZE_KV, False,   # DIAGONAL=False
...
def _attn_backward_cdb_bim_v3(
    ...
    # Bitmask-only is unsafe when a block straddles a doc boundary.
