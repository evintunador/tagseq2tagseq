"""
cross_doc_bitmask BIM v16 — L2-cache hints on bitmask loads.

Changes vs v12:
  Add cache_modifier=".cg" to every tl.load of q_bitmasks and kv_bitmasks
  in the forward and backward inner sub-kernels.

Rationale:
  q_bitmasks and kv_bitmasks are identical across all H heads (same BIM per
  batch item).  With H=16 and T=32k, the bitmask arrays are 2 × 256 KB = 512 KB
  total, well within A100's 40 MB L2.  Without cache hints, each head's CTA
  loads bitmask data with the default .ca policy (cache in L1+L2); since L1
  (~192 KB per SM) is too small to hold the full bitmask, the data keeps
  getting evicted and reloaded from L2.

  .cg (cache at global level = L2 only, bypass L1):
    1. Bitmasks land in L2 after the first head's loads and stay there.
    2. Subsequent heads (all 16) hit L2 instead of DRAM.
    3. L1 cache lines freed for K/V/Q/dLdO tiles, which benefit more from L1.

  Estimated gain: 1–3% on workloads with many cross-doc blocks (dense grant
  patterns where bitmask checks fire on every KV tile).

Implementation:
  New inner sub-kernels with the .cg bitmask loads:
    _attn_fwd_inner_cdb_v16   (fwd masked path)
    _bwd_kv_cdb_v16           (bwd dKV masked path)
    _bwd_q_cdb_v16            (bwd dQ masked path)

  New outer kernels that call the new inners:
    _attn_fwd_cdb_bim_v16
    _attn_backward_KV_cdb_bim_v16
    _attn_backward_Q_cdb_bim_v16

  The full-block (same-doc, no masking) paths are unchanged and still use
  v10's inner kernels directly.
"""
...
# ===========================================================================
# Forward inner — masked path with .cg bitmask loads
# ===========================================================================

@triton.jit
def _attn_fwd_inner_cdb_v16(
    Q, O, L, M,
    K_ptr, V_ptr,
    ...
    """Masked block (same_doc | in_grant | optional causal). .cg bitmask loads."""
    ...
    for start_KV in range(lo, hi, BLOCK_SIZE_KV):
        ...
        for c in tl.static_range(n_chunks):
            # .cg: cache bitmasks at L2 only, bypass L1
            q_bm  = tl.load(q_bitmasks_ptr  + c * T + offsets_QO_N,
                            mask=mask_QO_N, other=0, cache_modifier=".cg")
            kv_bm = tl.load(kv_bitmasks_ptr + c * T + offsets_KV_N,
                            mask=mask_KV_N, other=0, cache_modifier=".cg")
...
def _attn_fwd_cdb_bim_v16(
    ...
    # Full same-doc off-diagonal blocks: no masking (v10 inner, no bitmask loads)
    ...
    # Off-diagonal non-full blocks: .cg bitmask loads
    for i in range(n_full, num_kv - 1):
        kv_b = tl.load(q_kv_indices_ptr + q_kv_start + i)
        lo   = kv_b * BIM_BLOCK_SIZE
        O, L, M = _attn_fwd_inner_cdb_v16(
            Q, O, L, M, K_ptr, V_ptr, K_T_offsets, V_offsets,
...
# ===========================================================================
# Backward inner sub-kernels — masked paths with .cg bitmask loads
# ===========================================================================
...
def _bwd_kv_cdb_v16(
    ...
    """dLdK/dLdV: full masking. .cg bitmask loads."""
    ...
    for _ in range(num_steps):
        ...
        for c in tl.static_range(n_chunks):
            kv_bm = tl.load(kv_bitmasks_ptr + c * T + offsets_COL,
                            mask=offsets_COL < N, other=0, cache_modifier=".cg")
            q_bm  = tl.load(q_bitmasks_ptr  + c * T + offsets_ROW,
                            mask=mask_N, other=0, cache_modifier=".cg")
...
def _bwd_q_cdb_v16(
    ...
    """dLdQ: full masking. .cg bitmask loads."""
    offsets_ROW = start_ROW + tl.arange(0, BLOCK_SIZE_ROW)
    offsets_COL = start_COL + tl.arange(0, BLOCK_SIZE_COL)
    offsets_Dh  = tl.arange(0, Dh)
    ...
    for _ in range(num_steps):
        ...
        for c in tl.static_range(n_chunks):
            q_bm  = tl.load(q_bitmasks_ptr  + c * T + offsets_ROW,
                            mask=offsets_ROW < N, other=0, cache_modifier=".cg")
            kv_bm = tl.load(kv_bitmasks_ptr + c * T + offsets_COL,
                            mask=col_mask, other=0, cache_modifier=".cg")
            in_grant = in_grant | ((q_bm[:, None] & kv_bm[None, :]) != 0)
...
@triton.jit
def _attn_backward_KV_cdb_bim_v16(
    Q_ptr, K_ptr, V_ptr,
    ...
    """Compute dLdK and dLdV. .cg bitmask loads in masked paths."""
    ...
    # Diagonal (masked + causal) — uses .cg bitmask inner
    q_b_diag = tl.load(kv_q_indices_ptr + kv_q_start)
    dLdK, dLdV = _bwd_kv_cdb_v16(
        K, V, dLdK, dLdV,
    ...
    # Full same-doc Q blocks (no bitmask loads)
    ...
    # Off-diagonal non-full Q blocks — uses .cg bitmask inner
...
@triton.jit
def _attn_backward_Q_cdb_bim_v16(
    Q_ptr, K_ptr, V_ptr,
    dLdO_ptr, dLdQ_ptr,
    LSE_ptr, Delta_ptr,
    doc_ids_ptr, q_bitmasks_ptr, kv_bitmasks_ptr, T,
    ...
    """Compute dLdQ. .cg bitmask loads in masked paths."""
    ln2:  tl.constexpr = 0.6931471824645996
    rln2: tl.constexpr = 1.4426950408889634
    BLOCK_SIZE_MACRO: tl.constexpr = BIM_BLOCK_SIZE
    tl.static_assert(BLOCK_SIZE_MACRO % BLOCK_SIZE_MICRO == 0)
    ...
    # Full same-doc KV blocks (no bitmask loads)
    ...
    # Off-diagonal non-full KV blocks — uses .cg bitmask inner
    ...
    # Diagonal KV block (masked + causal) — uses .cg bitmask inner
...
class _CDBBIMv16(torch.autograd.Function):
    """v12 (BIM_BS=128) + .cg cache hints on all bitmask loads."""
    ...
    def forward(ctx, q, k, v, document_ids, q_bitmasks, kv_bitmasks, bim, scale):
        ...
        assert bim_bs == 128, f"v16 requires BIM_BLOCK_SIZE=128, got {bim_bs}"
        ...
        grid_fwd = (n_blocks, H)
        _attn_fwd_cdb_bim_v16[grid_fwd](
            q, k, v, O, LSE, scale,
            0, sH, sT, sDh,
    ...
    def backward(ctx, dLdO):
        ...
        _attn_backward_KV_cdb_bim_v16[grid](
        ...
        _attn_backward_Q_cdb_bim_v16[grid](
...
def triton_attn_cross_doc_bitmask_bim_v16(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    ...
    """Cross-doc BIM v16: v12 + .cg cache hints on bitmask loads."""
    if scale is None:
        scale = q.shape[-1] ** -0.5
    ...
    return _CDBBIMv16.apply(q, k, v, document_ids, q_bitmasks, kv_bitmasks, bim, scale)
