...
def _attn_backward_cdb_bim_v8(
    ...
    # Diagonal block — first entry in kv_q (MASK=True, full masking)
    # Use _attn_backward_KV_cdb so cross-doc grants within the diagonal block
    # (possible when the block straddles a doc boundary) are also handled.
    ...
    # Replaces the previous "contiguous same-doc range + partial BIM loop" split.
    # The split caused double-counting when a KV block straddles a doc boundary:
    # straddling blocks are classified as "non-full" in the BIM (not in n_full_kv),
    # so the same Q blocks ended up processed by both the contiguous range and the
    # BIM loop.  A single BIM pass avoids this entirely.
    ...
    # Diagonal block — last entry in q_kv (MASK=True, full masking)
    ...
    # Same rationale as STAGE1: replaces "contiguous same-doc + partial BIM loop"
    # split which double-counted straddling blocks.
