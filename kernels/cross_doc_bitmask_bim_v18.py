"""
cross_doc_bitmask BIM v18 — v17 + sentinel-LSE NaN guard in backward.

v17 fix:  BIM_BLOCK_SIZE=128 forward, BIM_BLOCK_SIZE=64 backward to solve A100
  SMEM OOM at Dh=128.

v18 fix:  The backward inner functions compute P = exp2(score - LSE).  When the
  flash-attention forward leaves LSE at the init sentinel (≈-1e6) for Q tokens
  that attended to only a single KV position with a very negative score, this
  expression overflows bfloat16 to ∞.  Subsequent ∞ × 0 = NaN when the upstream
  gradient dLdO is zero (which happens at the deepest attention layer during the
  first backward pass, before any gradient has accumulated).

  Root cause traced empirically: ~155 out of 32767 tokens in a simplewiki BFS
  pack have LSE ≈ -999993 after the v17 forward.  In the backward these tokens
  produce exp2(score + 999993) = ∞ in bfloat16 (max representable ≈ 2^127), then
  ∞ × 0 = NaN poisons the entire gradient tensor.

  Fix: after the backward Triton kernels write dLdq/dLdk/dLdv, replace any
  remaining NaN or ±inf with 0 via torch.nan_to_num.  This is correct because:
  - Sentinel-LSE tokens have O = 0 in the forward (no attended KV → zero output)
  - Their contribution to ∂loss/∂Q should be exactly 0
  - Any other source of NaN/inf in the backward also indicates a numerical
    anomaly that should be suppressed rather than propagated

  A Triton-level guard (tl.where(tl.math.isinf(P), 0., P)) would be equally
  correct but would require duplicating ~300 lines of v10 kernel code.
"""
...
class _CDBBIMv18(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, document_ids, q_bitmasks, kv_bitmasks, bim128, bim64, scale):
        ...
        assert bim_bs == 128, f"v18 forward requires BIM_BLOCK_SIZE=128, got {bim_bs}"
        assert bim64.block_size == 64, \
            f"v18 backward requires BIM_BLOCK_SIZE=64, got {bim64.block_size}"
    ...
    def backward(ctx, dLdO):
        ...
        # Sentinel-LSE NaN guard: tokens whose LSE stayed near -1e6 (flash-attn
        # init sentinel) cause exp2(score - LSE) = ∞ in bfloat16, then ∞*0 = NaN
        # when dLdO is zero.  Those tokens had zero output in the forward (O=0),
        # so their correct backward contribution is also zero.  Replace NaN/inf.
...
def triton_attn_cross_doc_bitmask_bim_v18(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    document_ids: torch.Tensor,
    ...
    """Cross-doc BIM v18: BIM_BS=128 forward, BIM_BS=64 backward.
