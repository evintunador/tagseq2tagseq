...
Supported for **all datasets**. Workers are partitioned so that linked documents land on the same shard (so BFS traversal doesn't immediately hit a shard boundary, which would degrade packs to `doc_causal`). The partitioner is auto-selected from the identifier format:

- **TheStack** (`owner/repo:path`): partition by repo prefix — repos are ground-truth communities.
- **Wikipedia / ArXiv** (flat identifiers): a graph-community partitioner (multi-source BFS Voronoi over undirected edges — seed `n_workers` docs, expand round-robin with a size cap to prevent hub nodes dominating, re-seed exhausted workers, round-robin leftovers). On the arxiv train split this keeps 61.9% of edges intra-shard at 16 balanced workers (vs 6.2% for random chunking). See the `data/epoch_precompute.py` module docstring for the algorithm.
