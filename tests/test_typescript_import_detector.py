"""
Tests for typescript_import_detector.py

Coverage: _parse_import_specs (import/from/side-effect/re-export/require/dynamic,
comment-blanking), _spec_to_detection_keys (index candidate, .js remap, bare
excluded), _resolve_relative_spec (./ , ../ , index, escape), detect_links
(specifier-space), detect_links_for_doc (path-resolved), index_doc_span.
"""
import pytest
import tiktoken
import torch

from model.graph_traversal.typescript_import_detector import (
    TypeScriptImportDetector,
    _parse_import_specs,
...
@pytest.fixture(scope="module")
def detector(enc):
    return TypeScriptImportDetector(decode_fn=enc.decode)
...
def test_type_only_import_creates_edge():
    assert _specs('import type { T } from "./types";\n') == ["./types"]
...
def test_detection_keys_strip_ext():
    assert _spec_to_detection_keys("./sub/b.ts") == {"sub/b", "sub/b/index"}
    assert _spec_to_detection_keys("./sub/b.tsx") == {"sub/b", "sub/b/index"}


def test_detection_keys_js_remap():
    # ./foo.js in TS means the sibling ./foo(.ts)
    assert _spec_to_detection_keys("./foo.js") == {"foo", "foo/index"}
...
# The detector's _resolve_relative_spec(spec, source_file_path) returns a list of
# repo-relative path candidates (bare + /index), most-specific first.

def test_resolve_same_dir():
    # importing file src/main importing ./helper -> [src/helper, src/helper/index]
    assert _resolve_relative_spec("./helper", "src/main") == ["src/helper", "src/helper/index"]


def test_resolve_updir():
    # importing file src/util/helper importing ../consts -> src/consts
    assert _resolve_relative_spec("../consts", "src/util/helper") == [
        "src/consts", "src/consts/index"]


def test_resolve_index_candidate_emitted():
    # ./models -> src/models AND src/models/index (directory-import candidate)
...
def test_detect_links_for_doc_resolves(enc, detector):
    src = 'import { VALUE } from "../consts";\n'
    ids = torch.tensor(enc.encode(src), dtype=torch.long)
    links = detector.detect_links_for_doc(ids, "repo:src/util/helper.ts")
    targets = {li.target_str for li in links}
    assert "src/consts" in targets
    assert "src/consts/index" in targets  # index candidate also emitted
...
def test_index_doc_span_strips_prefix_and_ext(detector):
    assert detector.index_doc_span(_Span("owner/repo:src/util/helper.ts")) == "src/util/helper"
