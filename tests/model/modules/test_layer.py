...
@pytest.mark.parametrize("n_embd", [256, 512])
def test_layer_mlp_dimensions(n_embd, device, dtype):
    if device != 'cuda':
        pytest.skip("FlexAttention requires CUDA")
    ...
    # FusedReLUSquaredMLP: hidden_dim = 4 * model_dim
    assert module.mlp.W1.shape == (4 * n_embd, n_embd)
    assert module.mlp.W2.shape == (4 * n_embd, n_embd)
