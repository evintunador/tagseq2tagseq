"""
TypeScript Pre-tokenizer: pre-tokenize the TypeScript file corpus into sharded
binary files using the path-keyed graph produced by
data/typescript_graph_extractor/build_typescript_graph.py.

Thin wrapper (arxiv/fineweb/go/java pattern): the builder already emitted
...
Usage:
    python -m data.pretokenize_typescript \\
        graphs/typescript/content.jsonl graphs/typescript/graph.jsonl \\
        -o runs/typescript_pretokenized -p 60
"""
import argparse
import json
import logging
...
def main():
    parser = argparse.ArgumentParser(
        prog="TypeScript Pre-tokenizer",
        description=__doc__,
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument("content_file", type=Path,
                        help="content.jsonl from build_typescript_graph.py")
    parser.add_argument("graph_file", type=Path,
                        help="graph.jsonl from build_typescript_graph.py")
