"""
JavaScript Pre-tokenizer: pre-tokenize the JavaScript file corpus into sharded
binary files using the path-keyed graph produced by
data/javascript_graph_extractor/build_javascript_graph.py.

Thin wrapper (arxiv/fineweb/go/java/typescript pattern): the builder already emitted
content.jsonl (one {"normed_identifier": <repo:path>, "content"} per file node),
so this reads it via the generic content source and calls the shared preprocessing
core.

Usage:
    python -m data.pretokenize_javascript \\
        graphs/javascript/content.jsonl graphs/javascript/graph.jsonl \\
        -o runs/javascript_pretokenized -p 60
"""
import argparse
import json
import logging
...
def main():
    parser = argparse.ArgumentParser(
        prog="JavaScript Pre-tokenizer",
        description=__doc__,
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument("content_file", type=Path,
                        help="content.jsonl from build_javascript_graph.py")
    parser.add_argument("graph_file", type=Path,
                        help="graph.jsonl from build_javascript_graph.py")
