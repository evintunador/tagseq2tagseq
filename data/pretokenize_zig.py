"""
Zig Pre-tokenizer: pre-tokenize the Zig file corpus into sharded binary files
...
Usage:
    python -m data.pretokenize_zig \\
        graphs/zig/content.jsonl graphs/zig/graph.jsonl \\
        -o runs/zig_pretokenized -p 60
"""
import argparse
import json
import logging
...
def main():
    parser = argparse.ArgumentParser(
        prog="Zig Pre-tokenizer",
        description=__doc__,
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument("content_file", type=Path,
                        help="content.jsonl from build_zig_graph.py")
    parser.add_argument("graph_file", type=Path,
                        help="graph.jsonl from build_zig_graph.py")
