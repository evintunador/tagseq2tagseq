"""
Rust Pre-tokenizer: pre-tokenize the Rust file corpus into sharded binary files
using the module-path-keyed graph produced by
data/rust_graph_extractor/build_rust_graph.py.

Thin wrapper (arxiv/fineweb/go/java pattern): the builder already emitted
content.jsonl (one {"normed_identifier": <repo@module_path>, "content"} per file
node), so this reads it via the generic content source and calls the shared
preprocessing core.

Usage:
    python -m data.pretokenize_rust \\
        graphs/rust/content.jsonl graphs/rust/graph.jsonl \\
        -o runs/rust_pretokenized -p 60
"""
import argparse
import json
import logging
...
def main():
    parser = argparse.ArgumentParser(
        prog="Rust Pre-tokenizer",
        description=__doc__,
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument("content_file", type=Path,
                        help="content.jsonl from build_rust_graph.py")
    parser.add_argument("graph_file", type=Path,
                        help="graph.jsonl from build_rust_graph.py")
