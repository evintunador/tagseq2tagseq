"""
Stream the Zig subset of bigcode/the-stack-dedup to a JSONL file.

Zig analogue of typescript_graph_extractor/download_typescript.py. Keeps only the
fields the builder needs, retaining ``.zig`` source files. PRESERVES repo/path
structure (relative-path resolution needs sibling files in the same repo).

Writes to /fss-data (never /fss for bulk I/O — see project memory).

Usage:
    python -m data.zig_graph_extractor.download_zig \\
        -o /fss-data/evin_t/tagseq2tagseq_artifacts/raw/zig/sample_zig.jsonl \\
        --limit 2000000
"""
...
def _zig_stream(token_enabled: bool = True):
    try:
        return load_dataset(
            "bigcode/the-stack-dedup", data_dir="data/zig",
...
def main(argv=None):
    ...
    logger.info("Streaming bigcode/the-stack-dedup data/zig -> %s (limit %d)",
    ...
    dt = time.time() - start
    logger.info("Done: wrote %d records (%d .zig files) in %.0fs",
                written, kept_zig, dt)
    print(json.dumps({"written": written, "zig_files": kept_zig, "seconds": round(dt)}))
    return 0
