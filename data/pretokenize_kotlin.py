"""
Kotlin Pre-tokenizer: pre-tokenize the Kotlin file corpus into sharded binary
files using the FQN-keyed graph produced by
data/kotlin_graph_extractor/build_kotlin_graph.py.

Thin wrapper (arxiv/fineweb/go/java pattern): the builder already emitted
content.jsonl (one {"normed_identifier": <FQN>, "content"} per declaration node),
so this reads it via the generic content source and calls the shared preprocessing
core.

Usage:
    python -m data.pretokenize_kotlin \\
        graphs/kotlin/content.jsonl graphs/kotlin/graph.jsonl \\
        -o runs/kotlin_pretokenized -p 60
"""
import argparse
import json
import logging
...
def main():
    parser = argparse.ArgumentParser(
        prog="Kotlin Pre-tokenizer",
        description=__doc__,
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument("content_file", type=Path,
                        help="content.jsonl from build_kotlin_graph.py")
    parser.add_argument("graph_file", type=Path,
                        help="graph.jsonl from build_kotlin_graph.py")
    ...
    logger.info(f"Loaded {len(graph_normed_ids):,} declaration nodes.")
