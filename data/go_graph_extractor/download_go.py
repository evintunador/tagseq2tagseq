"""
Stream the Go subset of bigcode/the-stack-dedup to a JSONL file.

Go analogue of github_graph_extractor/download_sample.py (which is hardcoded to
...
Usage:
    python -m data.go_graph_extractor.download_go \\
        -o /fss-data/evin_t/tagseq2tagseq_artifacts/raw/go/sample_go.jsonl \\
        --limit 2000000
"""
...
def main(argv=None):
    ...
    logger.info("Streaming bigcode/the-stack-dedup data/go -> %s (limit %d)",
    ...
    with open(args.output, "w", encoding="utf-8", buffering=8 * 1024 * 1024) as f:
        ...
        with tqdm(total=args.limit, unit="item", desc="download") as pbar:
            for item in islice(ds, args.limit):
                ...
                if not (path.endswith(".go") or path == "go.mod"
                        or path.endswith("/go.mod")):
                    continue
    ...
    logger.info("Done: wrote %d records (%d .go files) in %.0fs",
                written, kept_go, dt)
    print(json.dumps({"written": written, "go_files": kept_go, "seconds": round(dt)}))
    return 0
