"""
TypeScript dependency-graph builder for The Stack (TypeScript subset).

A TypeScript NODE is a source FILE, keyed by its repo-relative path WITHOUT
extension (``src/util/helper``). This is the file-node model (like Python/Java).
TS intra-repo imports are RELATIVE (``./foo``, ``../x/y``); a relative specifier
resolves deterministically to another file in the SAME repo by TS module
resolution rules (implemented here without touching a filesystem):

    ``./foo``  -> ``<dir>/foo.ts`` | ``foo.tsx`` | ``foo/index.ts`` | ...
    ``../x/y`` -> resolve ``..`` up a directory
    ``./foo.js`` -> ``<dir>/foo`` (.js->.ts remap: match the sibling .ts node)
...
deps that legitimately don't resolve and produce NO edge. ``.d.ts`` declaration
files are excluded from nodes (not real import targets).
...
``build_repo_nodes`` runs per-repo — no cross-repo contamination). Type-only
imports (``import type {...} from "./x"``), re-exports (``export ... from "./q"``),
CommonJS ``require("./z")`` and dynamic ``import("./x")`` all create edges.

Detection uses tree-sitter (build-time engine); the runtime
TypeScriptImportDetector re-derives the same imports from tokens and is graded
against the same tree-sitter oracle.

Outputs graph.jsonl + content.jsonl + graph_stats.json, consumed by
data.pretokenize_typescript via ContentJsonlSource.

Usage:
    python -m data.typescript_graph_extractor.build_typescript_graph \\
        raw/typescript/sample_typescript.jsonl -o graphs/typescript --min-links 1
"""
...
# Extensions that become NODES (source files). .d.ts excluded.
_NODE_EXTS = ("ts", "tsx")
...
class _TSParser:
    """tree-sitter TypeScript parser + relative-specifier extraction."""

    def __init__(self):
        import tree_sitter_typescript
        from tree_sitter import Language, Parser
        self._lang = Language(tree_sitter_typescript.language_typescript())
...
def build_repo_nodes(
    ...
    for path, content in repo_files:
        # only .ts/.tsx become nodes; skip .d.ts declaration files.
        ...
        key = _strip_ext(path)
        if key in nodes:
            continue  # first file wins on a key collision (e.g. foo.ts + foo.tsx)
...
def build(jsonl_path: Path, out_dir: Path, min_links: int = 1) -> dict:
    ...
    # A TS corpus is single-repo-per-pack (relative imports); the ``repo:path``
    ...
    logger.info("TypeScript graph: %d nodes, %d edges (%.2f avg out-degree) from %d repos",
