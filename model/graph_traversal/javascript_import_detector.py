"""
JavaScript Import Statement Link Detector

Detects JavaScript ``import`` / ``export ... from`` / CommonJS ``require`` /
dynamic ``import()`` statements in tokenized source and emits candidate file-path
links for the imported modules.

Node unit
---------
A JavaScript NODE is a source FILE (like Python/Java/TypeScript), keyed by its
repo-relative path WITHOUT extension (``src/util/helper``). Only RELATIVE
specifiers (``./foo``, ``../x/y``) are intra-repo and resolvable; bare specifiers
(``react``, ``lodash``) are external node_modules deps that legitimately do NOT
resolve to a corpus node.
...
``PythonImportDetector`` / ``TypeScriptImportDetector`` — this detector exposes TWO
methods:
...
* ``detect_links`` operates on a flat packed sequence with NO per-doc context, so
  ...
  (``./sub/b.js`` -> ``sub/b`` and ``sub/b/index``), used by the DETECTION-axis
...
JS resolution rules implemented (deterministic, no filesystem):
    ``./foo``  -> ``<dir>/foo``            (foo.js | foo.jsx | foo/index.js ...)
...
``import "./x"`` / re-export ``export {q} from "./q"`` and ``export * from "./r"`` /
CommonJS ``require("./z")`` (VERY common in JS) / dynamic ``import("./x")`` with a
literal string. Non-literal dynamic imports (``import(varName)``) and non-literal
requires (``require(varName)``) are dropped. JavaScript has NO type-only imports.

Detection mechanics mirror the other detectors: decode once, blank comments +
templates (so an import-looking string inside a comment or a backtick template is
not matched), regex the specifiers, map char offsets to token positions via a
cumulative per-token char index. JS uses ``//`` line comments and ``/* */`` block
comments (identical lexing to TS).
"""
...
def _spec_to_detection_keys(spec: str) -> Set[str]:
    ...
    (javascript_spec._spec_to_detection_keys) — they MUST agree on the same key
...
def _blank_comments_and_strings(text: str) -> str:
    """Blank JS comments to equal-length spaces; leave string bodies intact.
    ...
    template, but codegen templates commonly embed a ``require("./x")`` /
    ``import ... from "./x"`` STRING that is not a real import (tree-sitter treats
    it as template text), so blanking prevents that false positive.
    """
    out = list(text)
    i, n = 0, len(text)
    while i < n:
        ...
        # backtick templates, but a code-generator's template can contain a
        # `require("./x")` STRING that is not a real import — tree-sitter sees it
        # as template text, so we must blank it to avoid a false positive.
        if c == "`":
            out[i] = " "
            i += 1
            while i < n:
...
class JavaScriptImportDetector:
    """Detects JavaScript import statements in tokenized source sequences.
    ...
    def detect_links(self, input_ids: torch.Tensor) -> List[LinkInfo]:
        ...
        logger.debug("JavaScriptImportDetector: %d links from %d tokens",
    ...
    def detect_links_for_doc(
        ...
        logger.debug("JavaScriptImportDetector.detect_links_for_doc: %d links for %r",
